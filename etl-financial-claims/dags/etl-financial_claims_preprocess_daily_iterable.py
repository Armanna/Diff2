...
INITIAL_DATE_STR = "{{ dag_run.conf.get('INITIAL_DATE_STR') }}"
END_DATE_STR = "{{ dag_run.conf.get('END_DATE_STR') }}"

DATE_STR_LIST = []

"""
If you want to manually set report date to some date in the past start DAG with w/ сonfig: 
{
  "INITIAL_DATE_STR": "2024-07-02",
...
NUM_DAYS = (END_DATE - INITIAL_DATE).days
DATE_STR_LIST = [(INITIAL_DATE + timedelta(days=x)).strftime("%Y-%m-%d") for x in range(NUM_DAYS + 1)]

dag = DAG(
    'etl-financial-claims-preprocess_daily_iterable',
...
# Create tasks for first date outside the loop
ENV = {
    'PERIOD': 'day',
    'RUN_DATE': DATE_STR_LIST[0], 
...
run_tests = HippoGlueJobOperator(
    task_id=f'run_tests_{DATE_STR_LIST[0]}',
...
download_claims = HippoGlueJobOperator(
    task_id=f'download_claims_{DATE_STR_LIST[0]}',
...
download_sources = HippoGlueJobOperator(
    task_id=f'download_sources_{DATE_STR_LIST[0]}',
...
# Delete files from /temp prefix. 
"""
# Set dependencies for first date tasks
run_tests >> download_claims >> download_sources

# Create a DummyOperator to wait for all pipeline_list tasks to complete
all_tasks_done = DummyOperator(
    task_id='all_tasks_done',
    dag=dag
)
# Create tasks for rest of dates
for i, SINGLE_DATE in enumerate(DATE_STR_LIST):
    ENV['RUN_DATE'] = SINGLE_DATE
    ...
    download_sources >> financial_claims_preprocess >> all_tasks_done


# Set dependencies for wipe_temp_data_in_s3
all_tasks_done >> wipe_temp_data_in_s3
