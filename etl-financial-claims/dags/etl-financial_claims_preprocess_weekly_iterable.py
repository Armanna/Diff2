...
INITIAL_DATE_STR = Variable.get("etl-financial-claims-iterable-week-initial-date")
END_DATE_STR = Variable.get("etl-financial-claims-iterable-week-end-date") 
DATE_STR_LIST = []
...
MONDAY_START_DATE = (INITIAL_DATE - timedelta(days=INITIAL_DATE.weekday()))
MONDAY_END_DATE = (END_DATE - timedelta(days=END_DATE.weekday()))
NUM_WEEKS = int((MONDAY_END_DATE - MONDAY_START_DATE).days / 7)

DATE_STR_LIST = [(MONDAY_START_DATE + relativedelta(days=x * 7)).strftime("%Y-%m-%d") for x in range(NUM_WEEKS + 1)]

dag = DAG(
    'etl-financial-claims-preprocess_weekly_iterable',
...
for i, task in enumerate(pipeline_list):
    if i == 0:      ## Adding as run_tests as first task to complete
        run_tests >> download_claims >> download_sources >> task
...
"""
## Getting last task from pipeline
last_task = pipeline_list[len(pipeline_list)-1]

last_task >> wipe_temp_data_in_s3
